...
def compare_faces(id_photo_np: np.ndarray, selfie_np: np.ndarray) -> dict:
    ...
    # 3. Histogram comparison (Color distribution similarity or grayscale intensity similarity)
    id_hsv = cv2.cvtColor(id_face_resized, cv2.COLOR_BGR2HSV)
    selfie_hsv = cv2.cvtColor(selfie_face_resized, cv2.COLOR_BGR2HSV)
    
    # Check if either image is highly desaturated (e.g. black and white ID photo)
    id_sat = float(id_hsv[:, :, 1].mean())
    selfie_sat = float(selfie_hsv[:, :, 1].mean())
    
    if id_sat < 25.0 or selfie_sat < 25.0:
        # Grayscale comparison
        hist_id = cv2.calcHist([id_gray], [0], None, [64], [0, 256])
        hist_selfie = cv2.calcHist([selfie_gray], [0], None, [64], [0, 256])
    else:
        # HSV Color comparison
    ...
    # (i.e. > 0.45 is a match, scaled to 70%-99%)
    if final_score > 0.45:
    ...
    # Threshold for matching (lowered to 45.0% for better compatibility with desaturated/printed ID photos)
    matched = confidence_pct >= 45.0
